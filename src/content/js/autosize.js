/*!
  Autosize 3.0.2 modified by Paul Gear
  license: MIT
...
(function (global, factory) {
  ...
  function delayedUpdate(ta, retries) {
    if (!(ta && ta.nodeName && ta.nodeName === 'TEXTAREA')) {
      return;
    }
    if (!ta.offsetParent)
      return void window.setTimeout(delayedUpdate, 300, ta, --retries);
    var evt = document.createEvent('Event');
    evt.initEvent('autosize:update', true, false);
    ta.dispatchEvent(evt);
  }
  ...
  if (typeof window.getComputedStyle !== 'function') {
    ...
    autosize.delayedUpdate = function (el) {
      return el;
    };
  } else {
    autosize = function (el) {
    ...
    autosize.delayedUpdate = function (el, retries) {
      if (el && el.length) {
        Array.prototype.forEach.call(el, function(v) {
          delayedUpdate(v, retries);
        });
      } else if (el && el.nodeName) {
        delayedUpdate(el, retries);
      }
      return el;
    };
  }
